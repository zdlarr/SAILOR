...
opts.rendering_static = False # dynamic videos.

opts.data_name = 'SAILOR_test_dynamic_data0'
opts.ren_data_root    = './test_data/dynamic_data0/'

util.make_dir('./upsampling/results/')
...
for idx, data in enumerate(test_dataset):
    ...
    rgb   = F.fold(rgb, (opts.img_size, opts.img_size), (2,2), stride=2).permute(0,2,3,1)[0].numpy() # [B,H,W,3]
    ...
    # output data
    save_rgb_path = os.path.join( opts.output_dir, 'frame_' + name + '_' + str(vid.item()).zfill(3) + '_rgb.png' )
    Image.fromarray( (rgb * 255).astype(np.uint8) ).save(save_rgb_path)


fourcc = cv2.VideoWriter_fourcc('M', 'J', 'P', 'G')
# write video.
output_video_path = os.path.join(opts.output_dir, 'rgbs_{}.avi'.format(opts.data_name))
videowriter = cv2.VideoWriter(output_video_path, fourcc, 20, (1024, 1024), True)
frames = sorted(os.listdir(opts.output_dir))
for frame in frames:
    f_path = os.path.join(opts.output_dir, frame)
    image = cv2.imread(f_path)
    videowriter.write(image)
    print(frame + " has been written!")

videowriter.release()
print('fin video:', opts.data_name)