...
torch::Tensor sh_encode_forward(
    ...
    const uint32_t num_points = inputs.size(0); // B
    const uint32_t inputs_dim = inputs.size(1); // D
    const uint32_t outputs_dim = degree * degree; // deg ** 2;
    
    // define the output tensors. Get the output tensors.
    torch::Tensor outputs = torch::empty({num_points, outputs_dim}, inputs.options()); // [N_points, Output_dims]
...
torch::Tensor sh_encode_backward(
    ...
    torch::Tensor grad_inputs = torch::empty({num_points, inputs_dim}, grad.options());
