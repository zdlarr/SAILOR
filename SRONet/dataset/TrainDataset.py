...
class RenTrainDataset(object):
    ...
    def __init__(self, opts, phase='training'): # model phase: training, validating
        ...
        self.OCC_SAMPLE = os.path.join(self.root_dir, 'OCC_SAMPLE')
    ...
    def load_all_points(self, base_dir, subject, point_num):
        data_path = os.path.join(base_dir, subject, 'sample/samples.pkl')
        assert os.path.exists(data_path)

        with open(data_path, 'rb') as f:
            pts_data = pickle.load(f)
        ...
        pts = np.concatenate([pts_adp_p, pts_adp_n, pts_uni_p, pts_uni_n], axis=0).T # [3, N_points].
        pts_ov = np.concatenate([ #  inside is labeled as 1, else 0.
            np.ones([1, len(pts_adp_p)]), np.zeros([1, len(pts_adp_n)]),
            np.ones([1, len(pts_uni_p)]), np.zeros([1, len(pts_uni_n)]),
        ], axis=1) # [1, N_points].

        samples = torch.Tensor(pts).float() # [3, N_points.]
        labels = torch.Tensor(pts_ov).float() # [1, N_points.]
    ...
    def load_data(self, subject, num_views, target_num_views, yid, random_sample=False): # loading the RGBD images         
        ...
        if self.is_train:
            ...
            sample_points_data        = self.load_all_points(self.OCC_SAMPLE, subject, self.num_sampled_points * 3 // 4)
            sample_points_data_refine = self.load_all_points(self.OCC_SAMPLE_FINE, subject, self.num_sampled_points * 1 // 4) # OCC_SAMPLE_FINE for closer sampling points;
            
            sample_points_data['sampled_points'] = torch.cat( [sample_points_data['sampled_points'], sample_points_data_refine['sampled_points']], dim=1 )
            sample_points_data['point_labels']   = torch.cat( [sample_points_data['point_labels'], sample_points_data_refine['point_labels']], dim=1 )
            # get neighbors.
            neighbor_pts = sample_points_data['sampled_points'] + ( torch.rand_like(sample_points_data['sampled_points']) - 0.5 ) * 2.0 * self.opts.neighb_offset_render
            
            sample_points_data['sampled_points'] = torch.cat( [sample_points_data['sampled_points'], neighbor_pts], dim=1 )
            ...
            basic_info.update(sample_points_data)
...
if __name__ == '__main__':
    ...
    for data in train_dataset:
        ...
        sampled_pts = data['sampled_points']
        ...
        plt.show()
        # print(ks, rts)
        print(sampled_pts.shape, depths.shape)
