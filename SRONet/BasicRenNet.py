"""
    The basic Render Net. The input N views' videos (seperately processing for each frames).
    1. Using the Encoder(HRNet) to encoding the 2D features from input images.
    2. build the octree tree structure from the noisy depth inputs.
    3. for the sampled rays, intersect with the voxels. 
    4. sampling the points on the ray.
    5. peform Volume Rendering.
"""
import torch
...
from models.utils import reshape_multiview_tensor, DilateMask2, project3D, index2D
...
class BasicRenNet(nn.Module):
    ...
    def parse_input(self, images):
        ...
        # init variables.
        self._rgbs  = images[:, :3, ...].clone(); self._depths = images[:, 3:4, ...].clone(); self._masks = images[:, -1:, ...].clone() # [mask];
    ...
    def forward_ray_nerf(self, ray_oris, ray_dirs, num_points, target_calibs):
        ...
        rgbs, _ = self._nerf_func.forward_color( sampled_rgb_feats, sampled_rgbs, geo_feat, ray_dir_cam)
        
        # 3. rendering.
        if self.is_train: # pytorch version;
            output_rgbs, output_d, _, _ \
                    = self._volume_render_torch.forward_occ(sorted_depths, occ, rgbs, self.opts.valid_depth_max)
        else: # using CUDA-based volume rendering,
    ...
    def forward(self, input_images, calibs, sampled_points, batch_rays=None, target_calibs=None, target_masks=None):
        batch_rays_idx = None
        ...
        if sampled_points is not None:
            results_pifu = self.forward_query_occ(sampled_points)
            results = [results_nerf, results_pifu]
            return results, batch_rays_idx
