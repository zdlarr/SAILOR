"""
    The script for depth fusion method.
    Fuse the multi-views' depth 
"""
...
if __name__ == '__main__':
    ...
    base_dir = './real_dataset_3_14/boy_v0'

    frame_id = 'FRAME2384'
    
    rgbs_path = os.path.join(base_dir, 'RENDER', frame_id)
    rgbs_path = os.path.join(base_dir, 'COLOR', frame_id)
    depths_path = os.path.join(base_dir, 'DEPTH', frame_id)
    cam_params = os.path.join(base_dir, 'PARAM', frame_id)
    masks_path = os.path.join(base_dir, 'MASK', frame_id)
    output_path = './depths/fusion_882_raw.ply'

    rgb_datas = []; depth_datas = []; RTs = []; Ks = []

    for i, depth in enumerate(sorted(os.listdir(depths_path))):
        id = depth.split('.')[0]
        rgb_path = os.path.join(rgbs_path, id + '.jpg')
        ...
        mask_data   = cv2.imread(mask_path)[..., 0] / 255.0 # the front mask.
        ...
        rgb_data = np.ones([512,512,3]) # the gray rgb data.
        rgb_data[..., 0] = 150;
        rgb_data[..., 1] = 150;
        rgb_data[..., 2] = 200;
    ...
    print(rgb_datas.shape, depth_datas.shape, RTs.shape, Ks.shape)
